...
app.get('/v1/recommendation', async (req, res) => {
  try {
    ...
    const recQuery = `
      SELECT r.id, r.title, r.description, r.target_url, r.cta_text, n.id as advertiser_node_id
    ...
    if (recRes.rows.length === 0) {
      return res.status(404).json({ error: 'No available recommendations' });
    }
...
app.post('/v1/qau/verify', async (req, res) => {
  ...
  try {
    const { apiKey, recommendationId, advertiserNodeId, dwellTime, visitorIp, userAgent } = req.body;

    const pubRes = await client.query('SELECT id FROM nodes WHERE api_key = $1', [apiKey]);
    if (pubRes.rows.length === 0) return res.status(401).json({ error: 'Unauthorized' });
    const publisherId = pubRes.rows[0].id;

    if (dwellTime < 30) {
      return res.status(400).json({ status: 'Ignored', reason: 'Dwell time under 30s' });
    }

    const visitorHash = crypto.createHash('sha256').update(`${visitorIp || '127.0.0.1'}-${userAgent || 'mobile'}`).digest('hex');
    ...
    const qauRes = await client.query(
      ...
      [publisherId, advertiserNodeId, recommendationId, visitorHash, dwellTime]
    );
    const qauEventId = qauRes.rows[0].id;

    await client.query('UPDATE nodes SET credit_balance = credit_balance - 1.0 WHERE id = $1', [advertiserNodeId]);
    await client.query('UPDATE nodes SET credit_balance = credit_balance + 1.0 WHERE id = $1', [publisherId]);

    await client.query(
      `INSERT INTO credit_ledger (from_node_id, to_node_id, qau_event_id, amount, transaction_type)
       VALUES ($1, $2, $3, 1.0, 'QAU_EARNED')`,
      [advertiserNodeId, publisherId, qauEventId]
    );

    await client.query('COMMIT');
    res.json({ status: 'Success', message: 'Credit Transferred' });
  } catch (err) {
